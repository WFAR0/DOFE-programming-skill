...
A poll has a question, an array of options from which people can choose, and an array with the number of replies for each option.
 This data is stored in the starter 
object below.
...
If type is 'array', simply display the results array as it is, using console.log(). This should be the default option. If type is 
'string', display a string like "Poll results are 13, 2, 4, 1". 
4. Run the 'displayResults' method at the end of each 'registerNewAnswer' method call.

HINT: Use many of the tools you learned about in this and the last section 😉

BONUS: Use the 'displayResults' method to display the 2 arrays in the test data. Use both the 'array' and the 'string' option. Do NOT
 put the arrays in the poll object! So what shoud the this keyword look like in this situation?
...
const poll = {
  ...
  registerNewAnswer: function () {
    ...
    if (this.arr[answer] === undefined) {
      this.registerNewAnswer();
    } else {
      this.answers[this.arr[answer]] += 1;
  ...
  displayResults: function (type) {
    ...
    } else if (!Array.isArray(type) && typeof type === 'string') {
      console.log('string');
      console.log(`the answers are: ${this.answers.join(', ')}`);
    }
  },
};
document
